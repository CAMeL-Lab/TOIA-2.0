...
function AvatarSettings() {
	...
	var stream = [
		//Holds info on the stream
		{
			...
			privacy: "Public",
			language: "English",
			bio: "This is my professor album",
			ppl: "8",
			heart: "5",
			thumbs: "3",
		},
	...
	var avatars = [
		//This hold the video and information that are apart of the selected stream (Wahib)
		{
			still: sampleVideo,
			question:
				"This text serves as a placeholder for a question.This text serves as a placeholder for a question.",
		...
		},
		{
			still: sampleVideo,
			question:
				"This text serves as a placeholder for a question.This text serves as a placeholder for a question.This text serves as a placeholder for a question.",
			album: "default business personal",
		},
		{
			still: sampleVideo,
	...
	const handleClick = (event, index) => {
		//function that populates and depopulates the cardSelected variable and controls visibility of hidden menu
		let isChecked = event.target.checked;
		if (isChecked == true) {
			//checks if a video has been selected, adds to list and makes hidden menu appear
			cardSelected.push(avatars[index].question);
			...
			if (cardSelected.length == 0) {
				//checks if no video is being selected, hides hidden menu
				setDisplayItem("none");
			}
		}
	};
	...
	function groupSelect() {
		//fucntion called when the multiple videos are selected
	...
	function changehandler(classname, newVal) {
		//captures any changes to the stream info
		switch (classname) {
