...
function Player() {
        ...
        if (question.current == null || question.current == "") {
            ...
            axios.post(`/api/player`, {
                params: {
                    ...
                    record_log:"true",
                    ...(history.location.state.toiaID) && {interactor_id: history.location.state.toiaID},
                }
            }).then((res) => {
                ...
                if (res.data === "error") {
                    ...
                    setVideo(<video className="player-vid" id="vidmain" key={transcribedAudio}
                                    onEnded={interacting.current == 'false' ? fetchFiller : chatFiller} autoPlay>
                        <source src={res.data} onEnded={fetchFiller} type='video/mp4'></source>
                    </video>);
    ...
    function fetchFiller() {
        ...
        if (fillerPlaying) {
            axios.post(`/api/fillerVideo`, {
                ...
                let videoELem = <video muted className="player-vid" id="vidmain" key="filler"
                                       onEnded={interacting.current == 'false' ? fetchFiller : chatFiller} autoPlay>
        ...
        if (question.current != "" && newRating.current != 'false') {
            ...
            axios.post(`/api/player`, {
                ...
                if (res.data === "error") {
                    ...
                    newRating.current = 'false';
                    isFillerPlaying.current = 'false';
                    setVideo(<video className="player-vid" id="vidmain" key={transcribedAudio} onEnded={fetchFiller}
                                    autoPlay>
                        <source src={res.data} onEnded={fetchFiller} type='video/mp4'></source>
                    </video>);
    ...
    return (
        ...
        <button onClick={resetTranscript}>Reset</button> */}
                {(micMute) ? (
                    <button color='green' className="ui green microphone button mute-button" onClick={micStatusChange}>
                        <i aria-hidden="true" class="microphone icon"></i>{micString}</button>
                ) : (
                    <button className="ui secondary button mute-button" onClick={micStatusChange}><i aria-hidden="true"
                                                                                                     class="microphone slash icon"></i>{micString}
                    </button>
                )}

                <button className="ui red button skip-end-button"
