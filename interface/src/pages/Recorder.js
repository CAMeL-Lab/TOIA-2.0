					...
					<div className={"questionSuggestionsWrapper"}>
						<div className="cards-wrapper ui">
							<div className="ui cards">
								<RecordAVideoCard
									onClick={OnAddVideoCallback}
									isDisabled={recordNewVideoDisabled}
								/>
...
function Recorder() {
	...
	const [questionSuggestionModalActive, setQuestionSuggestionModalActive] =
		useState(false);
	...
	const [defaultStreamAlertActive, setDefaultStreamAlertActive] =
		useState(false);
		...
		if (history.location.state.isEditing) {
			...
			if (history.location.state.suggestedQuestion != null) {
				...
				if (history.location.state.id_question) {
					q = {
						...q,
						id_question: history.location.state.id_question,
					};
				}
	...
	const loadUserStreams = () => {
		return new Promise(resolve => {
			const options = {
				method: "GET",
				url: API_URLS.GET_USER_STREAMS(getUser().id),
			};
	...
	const fetchUserStats = () => {
		...
		axios
			...
			.then(function (response) {
				const { totalVideosCount, totalVideoDuration } =
					response.data.data;
				setVideosCount(totalVideosCount);
				setVideosTotalDuration(totalVideoDuration);
			})
	...
	const InitializeEditingMode = () => {
		...
		if (type && video_id) {
			...
			loadVideoData(video_id, type).then(video_data => {
				...
				const { answer, videoURL } = video_data;
				const videoPrivacy =
					video_data.private === true ? "Private" : "Public";
				...
				fetch(videoURL)
					...
					.then(blob => {
						...
						setAnswerProvided(answer);
						setPrivacyText(videoPrivacy);
						setPrivacySetting(video_data.private);
	...
	const loadVideoData = (video_id, type) => {
		return new Promise((resolve, reject) => {
			const options = {
				method: "GET",
				url: API_URLS.VIDEO_INFO(video_id),
			};
			...
			axios
				...
				.then(function (response) {
					...
					data.video_id = data.id_video;
					delete data.id_video;
					data.type = type;
	...
	const handleVideoTypeButtonClick = typeIndex => {
		...
		if (videoTypesJSON[typeIndex].auto_question) {
			...
			setQuestionsSelected([
				...questionsSelected,
				{ question: videoTypesJSON[typeIndex].displayText },
			]);
		} else {
						...
						<div className="video-layout-player-box">
							...
							<div className="layout video-layout-player-middle">
								<video
									...
									onDurationChange={() => {
										if (
											videoPlaybackRef.current.duration &&
