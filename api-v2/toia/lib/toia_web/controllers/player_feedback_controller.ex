defmodule ToiaWeb.PlayerFeedbackController do
  ...
  def create(conn, %{"player_feedback" => player_feedback_params}) do
    with {:ok, %PlayerFeedback{} = player_feedback} <- PlayerFeedbacks.create_player_feedback(player_feedback_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "player_feedback" => player_feedback_params}) do
    ...
    with {:ok, %PlayerFeedback{} = player_feedback} <- PlayerFeedbacks.update_player_feedback(player_feedback, player_feedback_params) do
      render(conn, :show, player_feedback: player_feedback)
    end
  end
