...
const RecordActivity = (user_id, activity, start_time, end_time = null, video_id = null, old_video_id = null) => {
    if (!(activity instanceof Activity)) throw "Tracker: Activity must be an instance of class Activity";
...
const RegisterLogin = (user_id, start_time = +new Date(), end_time = +new Date()) => {
...
const getLastActivityEndTime = (user_id, activity) => {
    ...
    return new Promise((resolve) => {
        ...
        connection.query(query, [user_id, activity.toString()], (err, entry) => {
            if (err) throw err;
            if (entry.length === 0) {
                resolve(-1);
            } else {
